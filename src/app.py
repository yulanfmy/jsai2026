...
def render_save_to_spotify(playlist: Playlist) -> None:
    """Show the Save to Spotify button and handle playlist creation."""
    has_creds = SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET
    if not has_creds:
        st.info(
            "Set `SPOTIFY_CLIENT_ID` and `SPOTIFY_CLIENT_SECRET` in your `.env` "
            "file to enable saving playlists to Spotify."
        )
        return
    ...
    if st.button("Save to Spotify", type="primary", key="save_spotify"):
        track_ids = [t.get("id", "") for t in playlist.all_tracks if t.get("id")]
        if not track_ids:
            st.error("No tracks with Spotify IDs found in this playlist.")
            return

        name = (
            f"MindTune: {playlist.current_emotion.name} → "
            f"{playlist.target_emotion.name}"
        )
        description = (
            f"Emotion transition playlist ({playlist.strategy_name} strategy) "
            f"generated by MindTune"
        )

        user_id = st.session_state.get("spotify_user_id", "")
        try:
            result = save_playlist(token, user_id, name, description, track_ids)
    ...
    with tab1:
        if current_name == target_name:
            ...
            if "playlist" in st.session_state:
                ...
                render_save_to_spotify(playlist)
